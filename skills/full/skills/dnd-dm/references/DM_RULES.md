...
4. 使用 Host 提供的有效权威状态切片或成功回执，确定行动相关能力、熟练、优势/
   劣势和 DC，直接调用已提供的工具。只有关键字段缺失、版本失效或受众改变时才补读。
   `character_query(view="list")` 只定位 ID，不能替代行动必需字段；完整角色卡仍是权威。
...
- 本地模式由 Host/Runtime 管理 revision、branch 和操作 ID；共享客户端保留显式版本。
  冲突不能盲重试、重掷骰或改换操作 ID；响应丢失时先恢复原操作回执。
- 角色卡、战役状态、场景进度、事件和记忆必须表达同一事实。
- 转移成功回执中的来源、目的地、队伍库存/钱包和新版本可直接用于叙述和后续动作。
  回执缺少必要字段或上下文失效时才补读对应实体，不固定执行写后读回。
...
`action.spell_resolution_id`。本地 Host 自动管理 revision；其他 Host 使用提交回执中的新 revision。
若命中打开 Shield 反应窗，
