...
class TrapService:
    def source_bound_trap_transition(
        ...
        *,
        ...
        if action in {"trigger", "escape"}:
            ...
            if action == "escape" and normalized_profile["profile_id"] != "srd5.1.falling_net":
                raise _support.CombatEngineError(
                    "this source-bound trap has no supported escape procedure"
                )
        elif action not in {"disable"} and (target_ids is not None or area_confirmed is not None):
            raise _support.CombatEngineError(
        ...
        if action == "disable" and normalized_profile["profile_id"] != "srd5.1.falling_net":
            raise _support.CombatEngineError(
    ...
    def _source_bound_trap_effect_transition(
        ...
        *,
        ...
        if profile_id == "srd5.1.poison_needle" and action == "trigger":
            if target_ids is not None:
                raise _support.CombatEngineError(
        ...
        if profile_id == "srd5.1.collapsing_roof" and action == "trigger":
            if (
                not isinstance(target_ids, list)
                or not target_ids
                or len(target_ids) > 20
        ...
        if action in {"trigger", "disable"}:
            ...
            if actor_id not in list(current_trap.get("restrained_actor_ids") or []):
                raise _support.CombatEngineError("actor is not restrained by this falling net")
            escape = normalized_profile["trigger"]["escape_check"]
            check_context = self.effective_rule_context(
                campaign_id,
                branch_id=branch_id,
                facts={
            ...
            if check.get("success") is True and actor_id in list(
                ...
                trap_state["traps"][trap_id] = current_trap
            result = {"check": check, "action": action}
            receipts = list(check.get("rule_receipts") or [])
