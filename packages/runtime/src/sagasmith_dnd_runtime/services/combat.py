        ...
        *,
        ...
        class CombatPlanRuntime:
            def __init__(self) -> None:
                self.encounter = _support.deepcopy(encounter)
            ...
            def record(self, actor_id: str) -> Any:
                if actor_id not in self.records:
                    self.records[actor_id] = runtime_services.require_campaign_actor(
                        campaign_id,
                        actor_id,
                    )
            ...
            def actor(self, actor_id: str) -> dict[str, Any]:
                actor = runtime_services.combat_actor_snapshot(actor_id)
                actor["sheet"] = _support.deepcopy(self.sheet(actor_id))
                actor["derived"] = runtime_services.derive_character_sheet(
            ...
            def set_sheet(self, actor_id: str, sheet: dict[str, Any]) -> None:
                ...
                runtime_services.sync_combatant_conditions(
                    self.encounter,
                    actor_id,
                    normalized,
                )
            ...
            def execute(
                ...
                *,
                ...
                if opcode == "roll.table":
                    ...
                    selected_roll = _support.asdict(_support.roll(f"1d{total_weight}"))
                    cursor = int(selected_roll["total"])
                    selected: Any = None
                    for entry in table:
                        cursor -= int(entry["weight"])
                        if cursor <= 0:
                            selected = _support.deepcopy(entry["value"])
                            break
                    return {
                        "roll": selected_roll,
                        "value": selected,
                ...
                if opcode == "target.validate":
                    ...
                    source = runtime_services.require_encounter_combatant(
                        self.encounter,
                        source_id,
                    ...
                    for raw_target_id in arguments["target_ids"]:
                        ...
                        target = runtime_services.require_encounter_combatant(
                            self.encounter,
                            target_id,
                ...
                if opcode == "check.contest":
                    return _support.resolve_actor_contest(
                        ...
                        source_rules=runtime_services.effective_rule_context(
                            campaign_id,
                            facts={
                        ...
                        target_rules=runtime_services.effective_rule_context(
                            campaign_id,
                            facts={
                ...
                if opcode == "damage.apply":
                    ...
                    for target_id_value in arguments["target_ids"]:
                        ...
                        combatant = runtime_services.require_encounter_combatant(
                            self.encounter,
                            target_id,
                        ...
                        runtime_services.add_concentration_window(
                            self.encounter,
                            target_id,
                ...
                if opcode == "healing.apply":
                    ...
                    for target_id_value in arguments["target_ids"]:
                        target_id = str(target_id_value)
                        runtime_services.require_healing_not_prevented(
                ...
                if opcode in {"actor.link", "actor.unlink"}:
                    ...
                    if opcode == "actor.link":
                        if existing is not None:
                            raise _support.CombatEngineError("semantic actor link already exists")
                        link = {
                            **identity,
                            "properties": _support.deepcopy(arguments.get("properties") or {}),
                            "plan_id": compiled_plan.id,
                            "step_id": step_id,
                        }
                        links.append(link)
                        return _support.deepcopy(link)
                    if existing is None:
                        raise _support.CombatEngineError("semantic actor link does not exist")
                ...
                if opcode == "knowledge.transfer":
                    ...
                    visible_ids = {
                        item.id
                        for item in runtime_services.knowledge.list(
                            campaign_id,
                            actor_id=source_id,
                            branch_id=resolved_branch_id,
                        )
