...
`application.py` owns permissions, revision checks, settlement and persistent
continuations. `operations.py` owns the typed registry and trusted command scope.
...
应用组装与处理器位于 `application.py`，类型化注册表与命令边界位于
