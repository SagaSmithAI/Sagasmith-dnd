    ...
    async def exercise() -> None:
        ...
        try:
                ...
                "combat_start",
                {
                    ...
                    "participant_config": [
                        ...
                        for index, (actor, position) in enumerate(
                            zip(
                                actors,
                                [(0, 0), (10, 0), (11, 0), (12, 0), (13, 0), (14, 0), (10, 1)],
                                strict=True,
                            )
                        )
                    ],
            ...
            try:
                ...
                assert targets[actors[2]["id"]]["affected"] is False
                assert (
                    targets[actors[2]["id"]]["skip_reason"] == "insufficient_remaining_hit_points"
                )
                assert targets[actors[3]["id"]]["skip_reason"] == "immune_to_magical_sleep"
                assert targets[actors[4]["id"]]["skip_reason"] == "immune_to_magical_sleep"
                assert targets[actors[5]["id"]]["skip_reason"] == "undead"
                assert targets[actors[6]["id"]]["skip_reason"] == "immune_to_charmed"
                assert (
                    result["result"]["pool_remaining"] == result["result"]["pool_roll"]["total"] - 1
                )
                close_server(server)
                server = create_server(config)
