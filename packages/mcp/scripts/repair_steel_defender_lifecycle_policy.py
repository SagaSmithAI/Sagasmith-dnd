"""Bind the exact pre-errata Eberron source to independent owner-death behavior.

This private repair preserves the source bytes and historical repair chain.
It neither substitutes the 2020 errata nor migrates existing campaign locks.
"""
...
_SOURCE_SHA = "6189467c53b675d39cc3eddd8de74932040ee2e2a9024a187b9dfd80f76b8cd4"
...
_PACKAGE_VERSION = "1.0.7-local.steel-defender-lifecycle.1"
_DEFINITION_VERSION = "1.0.5-local.steel-defender-lifecycle.1"
...
def repair_archive(data: bytes) -> tuple[bytes, dict]:
    ...
    if source_sha != _SOURCE_SHA:
        raise ValueError("archive is not the exact reviewed lifecycle input")
    package, blobs = loads_content_archive(data)
    validate_dnd_content_package(package)
    if (
        package["id"] != _PREFIX + ".addon"
        or package["version"] != "1.0.6-local.starting-equipment.1"
        or package["metadata"].get("distribution") != "private"
    ):
        raise ValueError("lifecycle input identity/version/distribution mismatch")
    ...
    feature = _one(
        ...
        "Steel Defender feature",
    )
    _one(
        (c for clause in feature.get("rule_clauses", [])
         for c in clause.get("source_citations", [])
         if c.get("source_ref", {}).get("chunk_key") == _CHUNK_KEY),
        "Steel Defender feature citation",
    )
    statblock = _one(
        (a for a in corrected["content"]["artifacts"]
         if a.get("id") == _PREFIX + ".statblock.steel-defender"),
        "Steel Defender statblock",
    )
    template = statblock["card"].get("dependent_actor_template")
    expected_binding = {
        "schema_version": 1, "kind": "feature_entitlement",
        "feature_artifact_id": feature_id, "relation_key": "steel_defender",
    }
    if not isinstance(template, dict) or template.get("owner_binding") != expected_binding:
        raise ValueError("reviewed Steel Defender owner binding mismatch")
    if "lifecycle_policy" in template:
        raise ValueError("historical template already declares a lifecycle policy")
    # This exact reviewed source predates the added owner-perishing sentence.
    # Interpretation is encoded here, never inferred from prose in the runtime.
    template["lifecycle_policy"] = {"schema_version": 1, "owner_death": "independent"}
    policy = dependent_actor_lifecycle_policy(template)
    selection = statblock["selection_contract"]
    statblock["selection_contract"] = build_selection_contract(
        statblock, status=selection["status"],
        references=selection.get("references", []), blockers=selection.get("blockers", []),
    )
    ...
    definition["definition_checksum"] = content_definition_checksum(
        ...
        artifacts=[a for a in corrected["content"]["artifacts"]
                   if a["rule_definition_id"] == definition["id"]],
        mechanics=[m for m in corrected["content"]["mechanics"]
                   if m.get("rule_definition_id") == definition["id"]],
    )
    statblock_id = statblock["id"]
    corrected["metadata"]["local_steel_defender_lifecycle_repair"] = {
        "source_archive_sha256": source_sha, "normalized_asset_sha256": _ASSET_SHA,
        "source_chunk_key": _CHUNK_KEY, "source_chunk_sha256": _CHUNK_SHA,
        "feature_artifact_id": feature_id, "changed_artifacts": [statblock_id],
        "lifecycle_policy": policy, "published": False,
    }
    ...
    if checked != rebuilt or checked_blobs != blobs:
        raise ValueError("lifecycle repair did not preserve archive roundtrip")
    return output, {
        "id": rebuilt["id"], "version": rebuilt["version"], "checksum": rebuilt["checksum"],
        "archive_sha256": hashlib.sha256(output).hexdigest(),
        "source_archive_sha256": source_sha, "changed_artifacts": [statblock_id],
        "lifecycle_policy": policy, "published": False,
    }
